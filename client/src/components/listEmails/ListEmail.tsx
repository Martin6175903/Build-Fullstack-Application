  ...
  return (
      <div className={'bg-zinc-700 p-5 rounded-lg text-2xl'}>
        ...
        <div>
          <div className={'flex gap-5 w-full'}>
            ...
            <p className={'w-full'}>{(isEdit ?
                <textarea value={editValue} onChange={e => setEditValue(e.target.value)} className={'w-full px-4 resize-none min-h-[85px]'}>
        ...
        <div className={'flex justify-between mt-5 pt-2 border-solid border-t-2 border-gray-500'}>
          ...
          <div>Date of sending the message: {`${isLen(date.getUTCDate())}.${isLen(date.getUTCMonth() + 1)}.${date.getUTCFullYear()}`}</div>
        </div>
      </div>
  );
};
